        ...
        for user in self.users.values_mut() {
            match user.balance.cmp(&0) {
                Ordering::Less => {
                    ...
                    let interest = abs_bal
                        .checked_mul(self.credit_interest)
                        .ok_or_else(|| format!("Interest calc overflow for {}", user.name))?
                        / 10000;
                    let intr_i64 = i64::try_from(interest)
                        .map_err(|_| format!("Interest too large for {}", user.name))?;
                    user.balance = user
                        .balance
                        .checked_sub(intr_i64)
                ...
                Ordering::Greater => {
                    let pos = u64::try_from(user.balance)
                        .map_err(|_| format!("Balance too large for {}", user.name))?;
                    let interest = pos
                        .checked_mul(self.debit_interest)
                        .ok_or_else(|| format!("Interest calc overflow for {}", user.name))?
                        / 10000;
                    let intr_i64 = i64::try_from(interest)
                        .map_err(|_| format!("Interest too large for {}", user.name))?;
    ...
    pub fn merge_bank(&mut self, other: Bank) -> Result<(), String> {
        for (n, u) in other.users {
            if let Some(eu) = self.users.get_mut(&n) {
                eu.balance = eu
                    .balance
                    .checked_add(u.balance)
                    .ok_or_else(|| format!("Balance overflow merging {}", n))?;
            } else {
                self.users.insert(n.clone(), u);
            }
        }
        self.credit_interest = self
            .credit_interest
            .checked_add(other.credit_interest)
            .ok_or_else(|| "Credit interest overflow".to_string())?
            / 2;
        self.debit_interest = self
            .debit_interest
            .checked_add(other.debit_interest)
            .ok_or_else(|| "Debit interest overflow".to_string())?
            / 2;
        Ok(())
    }
}
